...
class Mo_variables_ext
{
	...
	protected function current_url()
	{
		...
		$split = explode('.', $this->EE->input->server('HTTP_HOST'));
        
        if (isset($split[2])) {
            $this->set_global_var('url_domain', $split[1]);
        } else {
            $this->set_global_var('url_domain', $split[0]);
        }
        
        if (isset($split[2])) {
            $this->set_global_var('url_subdomain', $split[0]);
        } else {
            $this->set_global_var('url_subdomain', 'www');
        }
        
        $secure = !empty($_SERVER['HTTPS']);
        if ($secure == 1) {
            $this->set_global_var('url_full', 'https://'.$this->EE->input->server('HTTP_HOST'));
            $this->set_global_var('url_protocol', 'https://');
        } else {
            $this->set_global_var('url_full', 'http://'.$this->EE->input->server('HTTP_HOST'));
            $this->set_global_var('url_protocol', 'http://');
        }
        
        $this->set_global_var('host_ip', $this->EE->input->server('SERVER_ADDR'));
		...
		$query = ee()->db->select('channel_id, channel_name, channel_title, channel_url, search_results_url')->from('channels')->get();
		foreach ($query->result_array() AS $row) {
		    $url = str_replace('//', '/', '/'.$row['channel_url']);
		    $sub_url = str_replace('//', '/', '/'.$row['search_results_url']);
		    $this->set_global_var($row['channel_name'].'_name', $row['channel_title'], TRUE);
		    $this->set_global_var($row['channel_name'].'_channel_id', $row['channel_id'], TRUE);
		    $this->set_global_var($row['channel_name'].'_url', $url, TRUE);
		    $this->set_global_var($row['channel_name'].'_sub_url', $sub_url, TRUE);
		    
		    $active = '';
		    
		    if ($url == '/') {
		        if ((ee()->uri->segment(1) == '' OR ee()->uri->segment(1) == 'site' OR ee()->uri->segment(1) == 'index' OR ee()->uri->segment(1) == 'success') AND (ee()->uri->segment(2) == '' OR ee()->uri->segment(2) == 'index' OR ee()->uri->segment(2) == 'success')) {
		            $active = 1;
		        }
		    } else {
    		    $channel_segments = explode('/', $url);
    		    $match = 1;
    		    if (sizeof($channel_segments) > sizeof($segments)) {
    		        $match = 0;
    		    } else {
        		    foreach($channel_segments AS $key => $val) {
        		        if (isset($segments[$key]) AND $val != $segments[$key]) {
        		            $match = 0;
        		        }
        		    }
    		    }
		    ...
		    $this->set_global_var($row['channel_name'].'_is_active', $active, TRUE);
		}
	}
