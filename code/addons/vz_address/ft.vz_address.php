...
class Vz_address_ft extends EE_Fieldtype {
    ...
    {
        return ($name == 'channel' || $name == 'grid' || $name == 'blocks/1' || $name == 'fluid_field');
    }
    ...
    public function display_settings($settings)
    {
		...
		if (isset($settings['display_previous'])) {
			$display_previous = $settings['display_previous'];
		}
        ...
        $settings = array(
	        array(
	            'title' => 'vz_address_display_name',
	            'desc' => '',
	            'fields' => array(
	                'vz_address_display_name' => array(
	                    'type' => 'inline_radio',
	                    'value' => $display_name,
	                    'choices' => array(
		              'y' => 'Yes',
	        ...
	        array(
	            'title' => 'vz_address_display_previous',
	            'desc' => '',
	            'fields' => array(
	                'vz_address_display_previous' => array(
	                    'type' => 'inline_radio',
	                    'value' => $display_previous,
	                    'choices' => array(
		              'y' => 'Yes',
		              'n' => 'No'
		            )
        ...
        return array('field_options_vz_address' => array(
	        'label' => 'field_options',
	        'group' => 'vz_address',
	        'settings' => $settings
	    ));
    ...
    {
        return array(
            'display_name'     => empty($settings['vz_address_display_name']) ? '' : $settings['vz_address_display_name'],
            'display_previous' => empty($settings['vz_address_display_previous']) ? '' : $settings['vz_address_display_previous']
        );
    }
    ...
    private function _address_form($name, $data, $is_cell=false)
    {
        ...
        foreach(array_keys($this->fields) as $field)
        {
            ...
            if (($field == 'name' && $this->settings['display_name'] == 'y') OR $field != "name") {
        ...
        if (isset($this->settings['display_name']) && $this->settings['display_previous'] == 'y')
        {
            ...
            ee()->db->distinct();
            $query = ee()->db->get('exp_channel_data_field_'.$this->field_id)->result_array();
            ...
            if (count($query))
            {
                ...
                $form .= '<script type="text/javascript" src="/themes/ee/asset/javascript/compressed/jquery/jquery.js?v=1472053290"></script><script type="text/javascript">';
    ...
    public function save($data)
    {
        ...
        else
        {
			...
			if ($data['street'] != '') { $q .= $data['street'].', '; }
			if ($data['street_2'] != '') { $q .= $data['street_2'].', '; }
			if ($data['city'] != '') { $q .= $data['city'].', '; }
			if ($data['region'] != '') { $q .= $data['region'].', '; }
			if ($data['postal_code'] != '') { $q .= $data['postal_code'].', '; }
			if ($data['country'] != '') { $q .= $data['country'].', '; }
			
			$url = 'https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($q) . '&sensor=false';
			if (isset($this->settings['api_key']) AND $this->settings['api_key'] != '') {
                	$url .= "&key=".$this->settings['api_key'];
                }
			...
			if (isset($response['results'][0])) {
				$data['coords_x'] = $response['results'][0]['geometry']['location']['lat'];
				$data['coords_y'] = $response['results'][0]['geometry']['location']['lng'];
				$data['coords'] = $data['coords_x'].', '.$data['coords_y'];
			} else {
				$data['coords_x'] = '0';
				$data['coords_y'] = '0';
				$data['coords'] = '0, 0';
	...
	function display_global_settings()
	{
	    ...
	    return form_label('API Key', 'api_key').NBS.form_input('api_key', $api_key).NBS.NBS.NBS.' ';
	}
	...
	public function replace_coords($address, $params=array(), $tagdata=false)
    {
        return $address['coords'];
    }
	
	public function replace_coords_x($address, $params=array(), $tagdata=false)
    {
        return $address['coords_x'];
    }
	
	public function replace_coords_y($address, $params=array(), $tagdata=false)
    {
    ...
    {
    	if (isset($address['coords'])) {
            unset($address['coords']);
            unset($address['coords_x']);
            unset($address['coords_y']);
        }
    ...
    public function replace_static_map($address, $params=array(), $tagdata=false)
    {
        ...
        switch ($source)
        {
            ...
            default:
                ...
                if (isset($this->settings['api_key']) AND $this->settings['api_key'] != '') {
                	$output .= "&key=".$this->settings['api_key'];
                }
                break;
        }
    ...
    */
    public function update($current = '')
    {
        ...
        // Upgrade the Version
        ee()->db->update('fieldtypes', array('version' =>  $this->info['version']), array('name' => 'vz_address'));
