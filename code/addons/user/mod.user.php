...
use EEHarbor\User\FluxCapacitor\Base\Mod;
use EEHarbor\User\Library\AddonBuilderTrait;
use EEHarbor\User\Library\ChannelSync;
use EEHarbor\User\Library\Parser;
use EEHarbor\User\Library\Roles;
use EEHarbor\User\Library\ResetPassword;
use EEHarbor\User\Library\Utils;
...
class User extends Mod
{
    ...
    public function __construct()
    {
        ...
        if (ee()->config->item('req_mbr_activation') == 'manual' and
            ...
            foreach ($emails as $email) {
                ...
                if ($email->group_id == $email->Member->group_id) {
                    continue;
                }
        ...
        $this->lang_dir_user = SYSPATH . 'user/language/';
    ...
    public function profile()
    {
        ...
        $cond       = $member;
        $member_model = ee('Model')->get('Member', $this->member_id)->first()->toArray();

        foreach ($this->_mfields() as $key => $val) {
            $cond[$key] = (isset($member_model['m_field_id_'.$val['id']])) ?
                                $member_model['m_field_id_'.$val['id']] : '';
        }
        ...
        foreach ($this->_mfields() as $key => $val) {
            ...
            if (!empty($cond[$key])) {
                $fieldVal = ee()->typography->parse_type(
                    $cond[$key],
                    array(
                        'text_format'   => (!empty($val['format']) ? $val['format'] : 'text'),
                        'html_format'   => 'safe',
                        'auto_links'    => 'y',
                        'allow_img_url' => 'n'
                    )
        ...
        $tagdata = ee()->TMPL->parse_variables($tagdata, array($fields));

        $fields = array();

        foreach ($this->_mfields() as $key => $val) {
            $fields[$key] = (isset($cond[$key])) ? $cond[$key] : '';
        }
        ...
        foreach ($this->_mfields() as $key => $val) {
            ...
            $cond[ $val['name'] ]   = isset($member_model['m_field_id_'.$val['id']]) ? $member_model['m_field_id_'.$val['id']]: '';
            $tagdata                = ee()->functions->prep_conditionals($tagdata, $cond);
            ...
            if (empty($member_model['m_field_id_'.$val['id']])) {
    ...
    public function users()
    {
        ...
        if (ee()->TMPL->fetch_param('disable') !== false and stristr('member_data', ee()->TMPL->fetch_param('disable'))) {
            ...
            foreach ($this->_mfields() as $key => $val) {
                ...
                if (ee()->db->table_exists('member_data_field_' . $id)) {
                    $custom_field_selects .= ", md_$id.m_field_id_$id AS `$name`";
                    $custom_field_joins .= " LEFT JOIN exp_member_data_field_$id md_$id ON md_$id.member_id = m.member_id";
                }
            }

            $sql .= $custom_field_selects." FROM exp_members m LEFT JOIN exp_member_data md ON md.member_id = m.member_id".$custom_field_joins;
        }
    ...
    public function _users($inject = array())
    {
        ...
        foreach ($dates as $val) {
            if (strpos(ee()->TMPL->tagdata, LD.$val) === false) {
                continue;
            }
        ...
        foreach ($query_result as $count => $row) {
            ...
            foreach (ee()->TMPL->var_single as $key => $val) {
                ...
                if ($key == 'bio' && !empty($row['bio']) && is_string($row['bio']) && $row['bio'] != '') {
                ...
                //signature also needs love
                if ($key == 'signature' && !empty($row['signature']) && is_string($row['signature']) && $row['signature'] != '') {
            ...
            foreach ($this->mfields as $key => $val) {
                ...
                //  ----------------------------------------
                //  Parse Custom Variable Dates
                //  ----------------------------------------

                if (preg_match_all("/".LD.$key."\s+format=([\"'])([^\\1]*?)\\1".RD."/s", $tagdata, $matches)) {
                    for ($i=0, $s=count($matches[2]); $i < $s; ++$i) {
                        $str    = $matches[2][$i];
    ...
    public function edit()
    {
        ...
        foreach (ee()->TMPL->var_pair as $key => $val) {
            ...
            if ($key == 'languages') {
                ...
                if (is_dir($this->lang_dir_user) and
                    $fp = @opendir($this->lang_dir_user)) {
                    while (false !== ($file = readdir($fp))) {
                        if (is_dir($this->lang_dir_user.$file) and
                            substr($file, 0, 1) != ".") {
                            $dirs[] = $file;
                        }
                    }
        ...
        foreach ($this->_mfields() as $key => $val) {
            ...
            foreach (ee()->TMPL->var_pair as $k => $v) {
                if ($k == "select_".$key) {
                    ...
                    $tagdata = preg_replace(
                        "/" . LD . $k . RD . "(.*?)" . LD . preg_quote($this->t_slash, '/') . $k . RD . "/s",
                        str_replace('$', '\$', $this->_parse_select($key, $fields, $data)),
                        $tagdata
                    );
                }
            }
            ...
            $tagdata    = ee()->TMPL->swap_var_single(
                $key,
                isset($member_model['m_field_id_'.$val['id']]) ? $member_model['m_field_id_'.$val['id']]: '',
                $tagdata
            );
    ...
    public function edit_profile()
    {
        ...
        if (ee()->input->post('language') !== false and
            ...
            if (! is_dir($this->lang_dir.$language) && ! is_dir($this->lang_dir_user.$language)) {
                return $this->_output_error(
                    'general',
        ...
        if ($this->_param('required') !== false) {
            ...
            foreach ($required as $req) {
                if ($req == 'all_required') {
                    ...
                    elseif (in_array($req, $required_channel_fields)) {
        ...
        $catParents = array_unique($this->cat_parents);
        foreach ($catParents as $cat_id) {
            ee()->db->insert(
        ...
        */
        ...
        foreach ($this->_mfields() as $key => $val) {
            ...
            foreach ($custom_checks as $check) {
                if (isset($_POST[$check]) or $check != $key) {
                    continue;
                }
        ...
        if ($user_inputted_roles !== false &&
            ...
            } elseif (is_array($user_inputted_roles)) {
                $roles_input = $user_inputted_roles;
            }
        ...
        if (trim($this->_pref('member_update_admin_notification_template')) != '' and
    ...
    public function category_tree($type = 'text', $group_id = '', $parent_id = '', $only_show_selected = '')
    {
        ...
        foreach ($cat_array as $key => $val) {
            if (in_array($key, $this->used_cat) === true) {
                continue;
            }
    ...
    public function register()
    {
        ...
        if (ee()->config->item('require_captcha') == 'y') {
        ...
        foreach (ee()->TMPL->var_pair as $key => $val) {
            ...
            if ($key == 'select_member_groups') {
                if (ee()->TMPL->fetch_param('allowed_groups') !== false) {
                    ...
                    $tagdata    = preg_replace(
                        "/".LD.$key.RD."(.*?)".LD.preg_quote($this->t_slash, '/').$key.RD."/s",
            ...
            if ($key == 'languages') {
                ...
                if ($fp = @opendir($this->lang_dir_user)) {
                    while (false !== ($file = readdir($fp))) {
                        if (is_dir($this->lang_dir_user.$file) and substr($file, 0, 1) != ".") {
                            $dirs[] = $file;
                        }
                    }
                    closedir($fp);
    ...
    public function reg($remote = false)
    {
        ...
        if (ee()->session->userdata('member_id') != 0) {
            if ($this->_param('admin_register') !== 'yes' or
                (ee()->session->userdata['group_id'] != 1 and
                 ee()->session->userdata['can_create_members'] !== 'y')) {
                return $this->_output_error(
                    'general',
        ...
        foreach ($errors as $error_index => $error) {
            $errors[$error_index] = sprintf($error, $pw_length);
        }
        ...
        foreach ($optional as $key => $value) {
            if (isset($_POST[$value])) {
                $this->insert_data[$key] = ee()->input->post($value, true);
            } else {
                $this->insert_data[$key] = ee()->config->item($value);
            }
        }
        ...
        $this->insert_data['url'] = (!empty($_POST['url']) ? prep_url($_POST['url']) : '');
    ...
    {
        if (trim($message) == '' or trim(str_replace(',', '', $emails)) == '') {
            return false;
        }
        ...
        if (preg_match_all("/".LD."changed(.*?)".RD."(.*?)".LD.'\/changed'.RD."/s", $message, $matches)) {
            ...
            foreach ($old_data as $key => $value) {
                if (isset($new_data[$key]) && !is_array($new_data[$key]) && stripslashes($new_data[$key]) != $value) {
    ...
    public function do_search()
    {
        ...
        if (count($cfields) > 0 or count($keywords) > 0) {
            ...
            foreach ($this->_mfields() as $key => $val) {
                $id = $val['id'];
                if (ee()->db->table_exists('member_data_field_' . $id)) {
                    $sql    .= " LEFT JOIN exp_member_data_field_$id md_$id ON md_$id.member_id = m.member_id";
                }
            }
        }
        ...
        if (count($cfields) > 0) {
            ...
            foreach ($cfields as $key => $val) {
                if (!preg_match('/^m_field_id_(\d+)$/', $key, $matches)) {
                    continue;
                }
                ...
                if ($compare == 'like') {
                    $sql .= " ".$key." LIKE '%".ee()->db->escape_str($val)."%'".$andor;
                } else {
                    $sql .= " ".$key." = '".ee()->db->escape_str($val)."'".$andor;
                }
            }
        ...
        if (count($keywords) > 0) {
            ...
            foreach ($keywords as $keyword) {
                ...
                foreach ($this->_mfields() as $key => $val) {
                    $id = $val['id'];
                    if (in_array($key, $skip) || !ee()->db->table_exists("member_data_field_$id")) {
                        continue;
                    }

                    $sql .= " md_$id.m_field_id_$id LIKE '%".ee()->db->escape_str($keyword)."%'".$andor."\n";
                }
            }
        ...
        if (count($exclude) > 0) {
            ...
            foreach ($exclude as $ex) {
                ...
                foreach ($this->mfields as $key => $val) {
                    ...
                    $id = $val['id'];
                    $sql .= " md_$id.m_field_id_$id NOT LIKE '%".ee()->db->escape_str($keyword)."%'".$andor."\n";
                }
            }
        ...
        if ($fail === false) {
            ...
            // Add the custom fields to the SQL query
            foreach ($this->_mfields() as $key => $val) {
                $id = $val['id'];
                if (ee()->db->table_exists('member_data_field_' . $id)) {
                    $sql .= "md_$id.*, ";
            ...
            $sql .= "(m.total_entries + m.total_comments) AS total_combined_posts, mg.group_title, mg.group_description
                       FROM exp_members m
                       LEFT JOIN exp_member_data md ON md.member_id = m.member_id ";
    ...
    /**
     * Returns a JSON string with all authors matching a search term
     *
     * @return string
     */
    public function related_author_search()
    {
        if (!AJAX_REQUEST) {
            return '';
        }

        $searchString = ee()->input->get_post('search');
        ...
        if ($searchString) {
            $members->filter('screen_name', 'LIKE', '%' . $searchString . '%');
        }
        ...
        foreach ($members as $member) {
            $jsonData[] = array(
                'entry_id'      => $member->member_id,
                'title'         => $member->username,
                'channel_group' => $member->MemberGroup->group_id,
                'channel_name'  => $member->MemberGroup->group_title,
            );
        }
    ...
    public function _email_is_username($member_id = '0', $type = 'update')
    {
        ...
        if ($member_id == '0' and $type == 'update') {
            return false;
        }
        ...
        if (ee()->config->item('allow_username_change') != 'y' and $type == 'update') {
            return false;
        }
    ...
    protected function _upload_image($type = 'avatar', $member_id = 0, $test_mode = true)
    {
        ...
        if ($max_width < $width or $max_height < $height) {
            ...
            elseif (isset(ee()->image_lib->error_msg) and
    ...
    {
        if (is_object(ee()->TMPL) and $this->check_no(ee()->TMPL->fetch_param('log_views'))) {
            return false;
        }

        if ($this->member_id == 0 or $this->member_id == ee()->session->userdata('member_id')) {
            return false;
        }

        // Only update once per page load
        if (isset($this->cache[__FUNCTION__][$this->member_id])) {
            return false;
        }
    ...
    public function _search_fields($search_fields = array())
    {
        $sql = '';
    ...
    public function _parse_select($key = '', $row = array(), $data = '')
    {
        ...
        if (empty($this->mfields[$key]['list'])) {
            return '';
        }
        ...
        foreach (preg_split("/\r|\n/", $this->mfields[$key]['list']) as $val) {
            // Find out if the list items are in value/label pairs.
            if (strpos($val, '¡') !== false) {
                list($val, $label) = explode('¡', $val);
            }
            ...
            $out        = str_replace(LD."label".RD, $label, $out);
            $out        = str_replace(LD."option_index".RD, ++$count, $out);
            $out        = ee()->functions->prep_conditionals($out, array('value' => $val, 'label' => $label));
            $return     .= trim($out)."\n";
        }
    ...
    {
        if (count($form_data) == 0 && ! isset($this->form_data)) {
            return '';
        }

        if (! isset($this->form_data['tagdata']) || $this->form_data['tagdata'] == '') {
            $tagdata = ee()->TMPL->tagdata;
        } else {
            $tagdata = $this->form_data['tagdata'];
        ...
        if (ee()->TMPL->fetch_param('override_return') !== false && ee()->TMPL->fetch_param('override_return') != '') {
            ...
            } elseif (stristr($override_return, "http://") === false && stristr($override_return, "https://") === false) {
        ...
        foreach (array('return', 'RET') as $val) {
            if (isset($this->form_data[$val]) && $this->form_data[$val] !== false && $this->form_data[$val] != '') {
                ...
                } elseif (stristr($this->form_data[$val], "http://") === false && stristr($this->form_data[$val], "https://") === false) {
    ...
    {
        if (! isset($_POST['ACT']) or $this->_param('secure_action') != 'yes') {
            return;
        }
    ...
    protected function _output_error($type, $errors)
    {
        ...
        $template = ee('Model')->get('Template')->with('TemplateGroup')->filter('TemplateGroup.group_name', $x[0])->filter('template_name', $x[1])->first();
        ...
        // Set our new data into the global variables where the template parser can pick them up.
        ee()->config->_global_vars = array_merge(ee()->config->_global_vars, ee()->TMPL->global_vars, $data);

        // Parse our new template string into an EE template using EE's parser.
        ee()->TMPL->parse($template_data);

        // Generate the final template output with our new global vars.
    ...
    public function parse_timezone_menu_tag($tagdata = '', $timezone = 'UTC')
    {
        ...
        if ($matches) {
            ...
            foreach ($matches as $match_set) {
                ...
                $params = ee('Variables/Parser')->parseTagParameters($match_set[0]);
                ...
                foreach ($params as $param_name => $param_value) {
                    ...
                    //timezone
                    elseif (substr($param_name, 0, 9) == 'timezone:') {
