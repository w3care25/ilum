...
class ChannelSync extends AddonBuilderClass
{
    public $error                   = false;
    public $channel_id              = 0;
    ...
    public function submit_entry($options = array())
    {
        ...
        try {
            ...
            elseif (preg_match("/^[\d]{10}$/", trim((string) $_POST['entry_date']))) {
    ...
    protected function map_member_fields($member_id = 0, $map_to_post = true, $force_title = true)
    {
        ...
        if (! empty($map)) {
            foreach ($map as $member_field => $channel_field) {
                ...
                elseif (isset($member_data[$member_field])) {
        ...
        if (! isset($out['title']) && $force_title) {
            ...
            } elseif (isset($member_data['username'])) {
